"""
树莓派 + USB 摄像头 + 纯 OpenCV A4 黑框/圆心追踪 + 串口发送 X、Y 误差
...
- 识别到目标后，通过串口向 STM32 发送目标相对画面中心的 X、Y 误差；
...
树莓派 GPIO 串口接线（3.3V TTL）：
    GPIO14/TXD，物理引脚 8  ───> STM32 UART_RX
    树莓派 GND              ───> STM32 GND

如果使用 USB 转 TTL，把 SERIAL_PORT 改成 /dev/ttyUSB0。
树莓派 GPIO 与 STM32 均为 3.3V TTL，必须共地。
"""
...
# 60 FPS 下锁定曝光，避免自动曝光在高帧率时不断改变亮度、增益和黑框对比度。
# 多数 UVC 摄像头的 exposure_absolute 单位为 100 微秒：
# 100 约等于 10 ms；60 FPS 每帧约 16.7 ms，因此通常不要超过 160。
ENABLE_MANUAL_EXPOSURE = True
CAMERA_EXPOSURE_ABSOLUTE = 100
CAMERA_GAIN = 10
# V4L2 常见取值：0=关闭，1=50 Hz，2=60 Hz，3=自动。国内/新加坡通常用 50 Hz。
CAMERA_POWER_LINE_FREQUENCY = 1
# 关闭“低照度时自动降低帧率”，确保摄像头保持 60 FPS。
...
# 4. X、Y 误差串口参数
# ============================================================
ENABLE_ERROR_SERIAL = True
SERIAL_PORT = "/dev/ttyAMA0"
BAUD_RATE = 115200
DEBUG_UART = False

# 只发送 4 字节：EX_H EX_L EY_H EY_L
ERROR_SEND_INTERVAL_MS = 20
...
# X、Y 误差串口发送
# ============================================================
class ErrorSerialSender:
    """向 STM32 发送固定长度的目标中心误差帧。"""

    def __init__(self) -> None:
        self.port = None
        if not ENABLE_ERROR_SERIAL:
            print("误差串口已关闭：ENABLE_ERROR_SERIAL=False")
            return
        if serial is None:
            print("警告：未安装 pyserial，误差不会发送。")
            return
        try:
            ...
            time.sleep(0.2)
            print(f"误差串口已打开：{SERIAL_PORT} @ {BAUD_RATE}")
        except Exception as exc:
            print(f"警告：无法打开误差串口 {SERIAL_PORT}：{exc}")
            self.port = None
    ...
    def send_error(self, error_x: int, error_y: int) -> None:
        """只发送 X、Y 误差，各为 int16，高字节在前，共 4 字节。"""
        if not self.available:
            return

        error_x = max(-32768, min(32767, int(error_x)))
        error_y = max(-32768, min(32767, int(error_y)))
        ex_u16 = error_x & 0xFFFF
        ey_u16 = error_y & 0xFFFF
        ...
        try:
            ...
            if DEBUG_UART:
                print(
                    f"ERR TX EX={error_x} EY={error_y}:",
                    " ".join(f"{value:02X}" for value in frame),
                )
        except Exception as exc:
            print(f"串口发送失败：{exc}")
...
def _read_v4l2_controls() -> dict[str, tuple[int, int]]:
    """读取摄像头支持的 V4L2 控制及其范围。读取失败时返回空字典。"""
    if shutil.which("v4l2-ctl") is None:
        print("警告：未找到 v4l2-ctl，无法自动设置曝光。请安装：sudo apt install v4l-utils")
        return {}
    ...
    try:
        ...
        print(f"警告：读取摄像头控制失败：{exc}")
        return {}

    controls: dict[str, tuple[int, int]] = {}
    pattern = re.compile(
        r"^\s*([A-Za-z0-9_]+)\s+0x[0-9a-fA-F]+.*?min=(-?\d+)\s+max=(-?\d+)",
        re.MULTILINE,
    )
...
def _set_v4l2_control(
    ...
    if result.returncode != 0:
        ...
        print(f"警告：设置 {name}={value} 失败：{detail}")
        return False
...
def configure_camera_exposure() -> None:
    ...
    if CAMERA_AUTO_WHITE_BALANCE:
        _set_v4l2_control(controls, "white_balance_temperature_auto", 1)

    if ENABLE_MANUAL_EXPOSURE:
        # 你的摄像头使用 auto_exposure；部分摄像头使用 exposure_auto。
        manual_name = _set_first_supported_control(
            controls,
            ("auto_exposure", "exposure_auto"),
            1,  # Manual Mode
        )
        if manual_name is None:
            print("警告：没有找到手动曝光模式控制，曝光时间可能不会生效。")

        # 切换为手动曝光后再设置曝光时间。
        time.sleep(0.08)
        exposure_name = _set_first_supported_control(
            controls,
...
def draw_result(
    ...
    if result is not None:
        ...
        cv2.putText(
            out,
            f"X:{filtered_center[0] if filtered_center else 0} "
            f"Y:{filtered_center[1] if filtered_center else 0} "
    ...
    serial_text = "UART:ON" if serial_available else "UART:OFF"
    cv2.putText(
        out,
        f"FPS:{fps:.1f}  {serial_text}",
        (8, h - 12),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.55,
        (0, 255, 255),
...
def main() -> None:
    ...
    error_serial = ErrorSerialSender()
    preview = WebPreview()
    preview.start()
    ...
    try:
        while running:
            ...
            if result is not None:
                ...
                shown_target_x = target_cx
                shown_target_y = target_cy

                if (
                    lock_confirm_count >= LOCK_CONFIRM_FRAMES
                    and now - last_cmd_ms >= ERROR_SEND_INTERVAL_MS
                ):
                    last_cmd_ms = now
                    error_serial.send_error(error_x, error_y)

            else:
                if now - last_cmd_ms >= ERROR_SEND_INTERVAL_MS:
                    last_cmd_ms = now
                    error_serial.send_error(0, 0)
            ...
            if elapsed >= 1.0:
                ...
                print(
                    ...
                    f"X:{shown_target_x} Y:{shown_target_y} "
