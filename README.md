...
先只保留一个**状态枚举**、一个**信号结构体**和几个普通全局变量。这样结构最简单，也最容易移植到电赛小车任务代码里。
...
```c
TaskState_t task_state = TASK_WAIT;
```
...
typedef struct
{
    uint8_t start_flag;          // 启动按键

    uint8_t stop_point_flag;     // 检测到停车点
    uint8_t car_stable;          // 小车是否停稳

    uint8_t target_valid;        // 是否找到目标
    int16_t error_x;             // 目标水平误差
    int16_t error_y;             // 目标垂直误差

    uint8_t aim_finished;        // 是否瞄准完成
    uint8_t action_finished;     // 动作是否完成

    uint8_t reset_flag;          // 错误复位
